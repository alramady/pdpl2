...
const statusColors: Record<string, string> = {
  ...
  "مكتمل": "bg-secondary text-foreground dark:bg-secondary0/20 dark:text-muted-foreground",
};
  ...
  return (
    <Layout title="التسريبات" titleEn="Leaks">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <div onClick={() => { setSeverity(""); setSource(""); setStatus(""); }} className="rounded-xl p-4 border border-border bg-card cursor-pointer hover:shadow-md transition-all">
        ...
        <div onClick={() => { setSeverity("واسع النطاق"); setStatus(""); setSource(""); }} className="rounded-xl p-4 border border-border bg-card cursor-pointer hover:shadow-md transition-all">
          <div className="flex items-center gap-3 justify-end">
            <div className="text-right">
              <div className="text-2xl font-bold text-amber-600 dark:text-amber-400">واسعة</div>
              <div className="text-sm text-muted-foreground">واسعة النطاق</div>
            </div>
        ...
        <div onClick={() => { setStatus("قيد التحليل"); setSeverity(""); setSource(""); }} className="rounded-xl p-4 border border-border bg-card cursor-pointer hover:shadow-md transition-all">
          <div className="flex items-center gap-3 justify-end">
            <div className="text-right">
              <div className="text-2xl font-bold text-amber-600 dark:text-amber-400">تحليل</div>
              <div className="text-sm text-muted-foreground">قيد التحليل</div>
            </div>
            <div className="p-2 rounded-lg bg-amber-50 dark:bg-amber-500/10">
        ...
        <div onClick={() => { setStatus("مكتمل"); setSeverity(""); setSource(""); }} className="rounded-xl p-4 border border-border bg-card cursor-pointer hover:shadow-md transition-all">
          <div className="flex items-center gap-3 justify-end">
            <div className="text-right">
      ...
      <div className="rounded-xl p-4 border border-border mb-6 flex items-center gap-4 flex-wrap bg-card">
        <div className="flex-1 min-w-[200px] relative">
          ...
          <input
            ...
            className="w-full bg-secondary dark:bg-card/5 border border-border rounded-lg py-2 pr-10 pl-4 text-sm text-foreground dark:text-gray-200 placeholder-gray-400 focus:outline-none focus:border-teal-400"
          />
        </div>
        <select value={severity} onChange={(e) => { setSeverity(e.target.value); setPage(0); }} className="bg-secondary dark:bg-card/5 border border-border rounded-lg py-2 px-3 text-sm text-muted-foreground focus:outline-none">
        ...
        <select value={source} onChange={(e) => { setSource(e.target.value); setPage(0); }} className="bg-secondary dark:bg-card/5 border border-border rounded-lg py-2 px-3 text-sm text-muted-foreground focus:outline-none">
        ...
        <select value={status} onChange={(e) => { setStatus(e.target.value); setPage(0); }} className="bg-secondary dark:bg-card/5 border border-border rounded-lg py-2 px-3 text-sm text-muted-foreground focus:outline-none">
      ...
      <div className="space-y-3">
        {leaks.map((leak: any) => (
          <div
            ...
            className="rounded-xl p-5 border border-border hover:border-primary/30 dark:hover:border-teal-500/30 transition-all bg-card hover:shadow-sm cursor-pointer"
          >
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className={`text-xs px-2 py-0.5 rounded ${statusColors[leak.status] || "bg-secondary text-muted-foreground"}`}>
                  {leak.status}
                </span>
                <span className={`text-xs px-2 py-0.5 rounded ${sourceColors[leak.source] || "bg-secondary text-muted-foreground"}`}>
                  {leak.source}
                </span>
              </div>
              <div className="text-right flex-1 mr-4">
                <div className="flex items-center gap-2 justify-end">
                  <span className={`text-xs px-2 py-0.5 rounded border ${severityColors[leak.severity] || "bg-secondary text-muted-foreground border-border"}`}>
                    {leak.severity}
                  </span>
                  <h4 className="text-sm font-semibold text-foreground">{leak.title}</h4>
                </div>
            ...
            {leak.dataTypes && (
              <div className="flex items-center gap-2 mt-3 justify-end flex-wrap">
                {(Array.isArray(leak.dataTypes) ? leak.dataTypes : []).map((tag: string) => (
                  <span key={tag} className="text-[10px] px-2 py-0.5 rounded bg-secondary dark:bg-card/5 text-muted-foreground border border-border">
                    {tag}
                  </span>
                ))}
              </div>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-3 mt-6">
          <button
            ...
            className="p-2 rounded-lg border border-border disabled:opacity-30 hover:bg-secondary dark:hover:bg-accent/5 text-muted-foreground"
          >
            ...
            {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
              ...
              return (
                <button
                  ...
                  className={`w-8 h-8 rounded-lg text-sm font-medium ${page === pageNum ? "bg-teal-600 text-white" : "text-muted-foreground hover:bg-secondary dark:hover:bg-accent/5"}`}
                >
                  {pageNum + 1}
                </button>
              );
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={() => setSelectedLeak(null)}>
          <div className="bg-card rounded-2xl shadow-2xl border border-border w-full max-w-2xl max-h-[85vh] overflow-hidden" onClick={e => e.stopPropagation()} dir="rtl">
            <div className="flex items-center justify-between p-5 border-b border-border">
              <button onClick={() => setSelectedLeak(null)} className="p-1.5 rounded-lg hover:bg-accent text-muted-foreground">
                <X size={18} />
              </button>
              <h3 className="text-lg font-bold text-foreground">تفاصيل التسريب</h3>
            </div>
            <div className="p-5 overflow-y-auto max-h-[70vh]">
              <h4 className="text-base font-bold text-foreground mb-3">{selectedLeak.title}</h4>
              ...
              <div className="grid grid-cols-2 gap-3 mb-5">
                <div className="p-3 rounded-lg bg-red-50 dark:bg-red-500/10 text-center">
                  ...
                  <div className="text-xs text-muted-foreground">سجل مكشوف</div>
                </div>
                <div className="p-3 rounded-lg bg-amber-50 dark:bg-amber-500/10 text-center">
                  <div className={`text-sm font-bold px-2 py-1 rounded inline-block ${severityColors[selectedLeak.severity] || "bg-secondary text-muted-foreground"}`}>
                    {selectedLeak.severity}
                  </div>
                  <div className="text-xs text-muted-foreground mt-1">مستوى الخطورة</div>
                </div>
              </div>

              <div className="space-y-3 mb-5">
                <div className="flex items-center justify-between p-3 rounded-lg bg-secondary border border-border">
                  <span className={`text-xs px-2 py-0.5 rounded ${sourceColors[selectedLeak.source] || "bg-secondary text-muted-foreground"}`}>{selectedLeak.source}</span>
                  <span className="text-sm text-muted-foreground">المصدر</span>
                </div>
                <div className="flex items-center justify-between p-3 rounded-lg bg-secondary border border-border">
                  <span className={`text-xs px-2 py-0.5 rounded ${statusColors[selectedLeak.status] || "bg-secondary text-muted-foreground"}`}>{selectedLeak.status}</span>
                  <span className="text-sm text-muted-foreground">الحالة</span>
                </div>
                <div className="flex items-center justify-between p-3 rounded-lg bg-secondary border border-border">
                  <span className="text-sm text-foreground">{new Date(selectedLeak.discoveredAt).toLocaleDateString("ar-SA", { year: "numeric", month: "long", day: "numeric" })}</span>
                  <span className="text-sm text-muted-foreground">تاريخ الاكتشاف</span>
                </div>
                {selectedLeak.sourceUrl && (
                  <div className="flex items-center justify-between p-3 rounded-lg bg-secondary border border-border">
                    ...
                    <span className="text-sm text-muted-foreground">رابط المصدر</span>
                  </div>
                )}
              </div>
              ...
              {selectedLeak.description && (
                <div className="mb-5">
                  ...
                  <p className="text-sm text-muted-foreground leading-relaxed">{String(selectedLeak.description)}</p>
                </div>
              )}

              {selectedLeak.dataTypes && Array.isArray(selectedLeak.dataTypes) && selectedLeak.dataTypes.length > 0 && (
                <div className="mb-5">
                  <h5 className="text-sm font-semibold text-foreground mb-2">أنواع البيانات المسربة</h5>
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  className="flex-1 py-2.5 rounded-lg border border-border text-foreground font-medium hover:bg-secondary dark:hover:bg-accent/5 transition-colors text-sm"
                >
                  سلسلة الأدلة
                </button>
              </div>
