  ...
  return (
    <Layout title="مواقع اللصق" titleEn="Paste Sites">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <div className="rounded-xl p-4 border border-border bg-card">
          <div className="flex items-center gap-3 justify-end">
            <div className="text-right">
              ...
              <div className="text-sm text-muted-foreground">إجمالي المراقبة</div>
            </div>
        ...
        <div onClick={() => { setThreatLevel(""); setPage(0); }} className="rounded-xl p-4 border border-border bg-card cursor-pointer hover:shadow-md transition-all">
          <div className="flex items-center gap-3 justify-end">
            <div className="text-right">
      ...
      <div className="space-y-3">
        {items.map((item: any) => (
          <div key={item.id} onClick={() => setSelected(item)} className="rounded-xl p-5 border border-border bg-card hover:border-primary/30 transition-all cursor-pointer hover:shadow-sm">
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-2">
                <span className="text-xs text-muted-foreground">{new Date(item.detectedAt).toLocaleDateString("ar-SA")}</span>
              ...
              <div className="text-right flex-1 mr-4">
                ...
                <p className="text-xs text-muted-foreground mt-0.5 font-mono">{item.pasteUrl ? String(item.pasteUrl).slice(0, 50) : ""}</p>
              </div>
            </div>
            {item.content && (
              <p className="text-xs text-muted-foreground mt-2 text-right">{String(item.content).slice(0, 100)}...</p>
            )}
          </div>
        ))}
      </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={() => setSelected(null)}>
          <div className="bg-card rounded-2xl shadow-2xl border border-border w-full max-w-2xl max-h-[85vh] overflow-hidden" onClick={e => e.stopPropagation()} dir="rtl">
            <div className="flex items-center justify-between p-5 border-b border-border">
              ...
              <h3 className="text-lg font-bold text-foreground">تفاصيل اللصق</h3>
            </div>
            <div className="p-5 overflow-y-auto max-h-[70vh]">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground font-mono">{selected.pasteUrl ? String(selected.pasteUrl) : ""}</p>
                </div>
              </div>
              <div className="grid grid-cols-3 gap-3 mb-5">
                ...
                <div className="p-3 rounded-lg bg-secondary border border-border text-center">
                  <div className="text-sm font-bold text-foreground">{new Date(selected.detectedAt).toLocaleDateString("ar-SA")}</div>
                  <div className="text-xs text-muted-foreground mt-1">تاريخ الاكتشاف</div>
                </div>
              </div>
              {selected.content && (
                <div className="mb-5">
                  <h5 className="text-sm font-semibold text-foreground mb-2">المحتوى</h5>
                  <pre className="text-xs text-muted-foreground bg-secondary p-4 rounded-lg border border-border overflow-x-auto whitespace-pre-wrap font-mono leading-relaxed max-h-64">{String(selected.content)}</pre>
                </div>
              )}
              {selected.keywords && (
                <div className="mb-5">
