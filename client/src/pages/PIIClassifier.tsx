...
const categoryColors: Record<string, string> = {
  "اتصال": "bg-teal-500/20 text-teal-400",
...
export default function PIIClassifier() {
  ...
  return (
    <Layout title="مصنّف PII" titleEn="PII Classifier">
      {/* Header */}
      <div className="rounded-xl p-6 mb-6 border border-white/5 relative overflow-hidden" style={{ background: "linear-gradient(135deg, #0d1117 0%, #1a1a2e 50%, #0d1117 100%)" }}>
        <div className="flex items-center justify-between">
          <div />
          <div className="text-right flex items-center gap-4">
            <div>
              <h2 className="text-xl font-bold text-white">مصنّف البيانات الشخصية</h2>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <div className="rounded-xl p-4 border border-white/5" style={{ background: "#111827" }}>
          <div className="text-2xl font-bold text-teal-400">{allPiiTypes.length}</div>
      ...
      <div className="flex items-center gap-3 mb-6">
        <div className="flex-1 relative">
          ...
          <input type="text" placeholder="البحث في أنواع البيانات..." className="w-full bg-[#111827] border border-white/10 rounded-lg py-2 pr-10 pl-4 text-sm text-gray-200 placeholder-gray-500 focus:outline-none focus:border-teal-500/50" />
        </div>
            ...
            <tbody>
              {allPiiTypes.map((pii, i) => (
                <tr key={i} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors cursor-pointer">
                  ...
                  <td className="p-4 text-center">
                    <span className="text-sm font-semibold text-white">{pii.count}</span>
                  </td>
                  <td className="p-4">
                    <div className="w-24 h-2 rounded-full bg-white/5 overflow-hidden mx-auto">
